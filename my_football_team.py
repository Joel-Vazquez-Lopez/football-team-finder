...
st.set_page_config(page_title="Football Team Finder", layout="centered")
...
# st.title("Find Your Football Team")
...
if st.session_state.step == 1:
    ...
    st.header("Setup your test")

    mode = st.selectbox(
        "Choose category",
        ["All teams", "LaLiga", "Premier League", "Bundesliga", "National Teams", "Inazuma Eleven"]
    )

    length = st.radio(
        "Test length",
        ["Quick (10)", "Medium (20)", "Long (30)"]
    )
    ...
    if st.button("Start"):
        ...
        if "10" in length:
            ...
            st.session_state.NUM_QUESTIONS = 30

        # important: don't exceed available questions
        num_q = min(st.session_state.NUM_QUESTIONS, len(questions))
    ...
    st.markdown(
        "<h1 style='text-align:center; margin-bottom:10px;'>Find Your Football Team</h1>",
        unsafe_allow_html=True
    )

    st.markdown(
        f"<h2 style='text-align:center; color:#aaa;'>Question {q_index+1}/{total}</h2>",
        unsafe_allow_html=True
    )
    ...
    st.markdown(
        f"<h3 style='text-align:center; margin-top:40px; margin-bottom:30px;'>"
        f"{q['question']}</h3>",
        unsafe_allow_html=True
    )

    # =========================
    # OPTIONS (CENTERED BOX)
    # =========================
    ...
    with col2:
        b1, b2 = st.columns(2)
        next_clicked = b1.button("Next", use_container_width=True, disabled=not choice)
        back_clicked = b2.button("Back", use_container_width=True)
...
# =========================
    ...
    categories = {
        "LaLiga": teams,
        "Premier League": teams_epl,
        "Bundesliga": teams_bundesliga,
        "National Teams": teams_national,
        "Inazuma Eleven": teams_inazuma
    }

    selected_teams = all_teams if mode == "All teams" else categories[mode]

    user = np.zeros(8)

    for i, ans in enumerate(st.session_state.answers):
        vec = st.session_state.questions[i]["options"][ans]["vector"]
        user += np.array(vec)
    ...
    for team, vec in selected_teams.items():
        vec = np.array(vec)
        vec = vec / np.linalg.norm(vec)

        similarity = cosine_similarity([user], [vec])[0][0]
        similarity += random.uniform(-0.02, 0.02)

        results.append((team, similarity))

    results = sorted(results, key=lambda x: x[1], reverse=True)

    st.success(f"Your team is: {results[0][0]}")

    st.subheader("Top matches:")
    for team, score in results[:5]:
        st.write(f"{team}: {round(score*100,1)}%")
